	...
	for(int index = 0; index<iron_man->elements; index++) {
		...
		while(count >= 1) {
			if(refresh == 2 && color != background_color) {
				//drowPixel(Xpos + j, Ypos + i, color, 3);				//it can be used to increase the image but the emulator is very slow
				LCD_SetPoint( Xpos + j, Ypos + i, color);					//original position
			}
...
/******************************************************************************
* Function Name  : drawSpeakerIcon
* Description    : It paints the image of food or snack selected
* Parameters     : Coordinate where the speaker icon must be painted
* Return         : None
* Attention		 	 : None
*******************************************************************************/
void drawSpeakerIcon(uint16_t x, uint16_t y) {
	
	Box_Coordinate coordinate;
	coordinate.x_0 = x;				coordinate.x_1 = x+24;
	coordinate.y_0 = y-24;		coordinate.y_1 = y+14;
	
	LCD_ClearFaster(&coordinate, top_background_color);
	
	//it paints a little square
	for(uint16_t i=y; i<y+6; i++) {
		for(uint16_t j=x; j<x+6; j++) {
			drowPixel(j, i, Black, 1);
		}
	}
	...
	//it paints a trapeze
	for(uint16_t j=x; j<x+4; j++) {
		z++;
		for(uint16_t i=y-z; i<y+6+z; i++) {
			drowPixel(j, i, Black, 1);
		}
	}
	...
	if((int) volume <= 0) {
		level = 0;
	} else if((int) volume > 0 && volume < 4) {
		level = 1;
	} else if((int) volume >= 4 && volume < 6) {
		level = 2;
	} else if((int) volume >= 6 && volume < 8) {
		level = 3;
	} else if((int) volume >= 8) {
		level = 4;
	}
	
	//it paints some semi-circles
	for(uint16_t j=x; j<x+level*3; j+=3) {
		z += 2;
		
		for(uint16_t i=y-z; i<y+z; i++) {
			drowPixel(j, i, Black, 1);
			drowPixel(j-1, i, Black, 1);
		}
