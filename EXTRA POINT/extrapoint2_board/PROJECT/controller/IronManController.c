...
/*		SOUND						*/
volatile uint8_t execute_song = 0;
...
**
** Descriptions:		Update the speaker icon
**
** parameters:				None
** Returned value:		None
**
...
void ironManRunAway() {
	...
	execute_song = 3;				//this will enable the song
...
**
** parameters:			go_to_eat_animation it specifies if iron man has to go to eat
** Returned value:	None
**
******************************************************************************/
void increaseSnackLevel(uint8_t go_to_eat_animation) {
	writeMealAndSnack(0);
...
void setDisplay() {
	...
	updateVolume();		//paint the volume level on the display
