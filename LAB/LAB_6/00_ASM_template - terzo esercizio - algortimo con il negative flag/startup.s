...
loop
				...
				MOV count, #0					;update count to 0
				LSL var, #1
				CMP value, var
				MOV value, var
				BMI check						;if(N == 1) goto check
				B loop		
...
check
				...
				BEQ exit						;if(index == 0) goto exit
				LSL var, #1
				CMP value, var					;if(N == 0) goto loop (another 1 so it's only one 0)
				MOV value, var
				BPL	loop

				ADD count, count, #1			;count++ (I count the additional calculate done in the check section)
				
counter
				SUB index, index, #1
				CMP index, #0
				BEQ exit						;if(count == 0) goto exit
				
				LSL var, #1
				CMP value, var
				MOV value, var
				BPL exit						;if(N == 0) goto exit (I got a 1 therefore I terminate all leading zeros)
				
				ADD count, count, #1			;else count++
				B counter
